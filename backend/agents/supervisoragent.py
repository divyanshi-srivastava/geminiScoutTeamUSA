...
supervisor_agent = Agent(
    ...
    instruction=(
        ...
        "═══════════════════════════════════════════\n"
        "The user has completed the interview. You MUST call agents in STRICT SEQUENCE — each step must "
        "complete before the next begins. NEVER call narrator_agent or compliance_agent before scout_agent finishes.\n"
        "If [SYSTEM: TIME_TRAVEL] and [SYSTEM: AGE_OVERRIDE] headers are present, pass them to all delegates.\n"
        "STEP 1 — Call `scout_agent` FIRST. Pass the full system header including biometrics and AGE_OVERRIDE "
        "if present. WAIT for scout_agent to return its JSON array of TWO archetype profiles before proceeding.\n"
        "STEP 2 — Only after scout_agent has returned its JSON: Call `narrator_agent`. Pass the scout's JSON "
        "array AND the full conversation history including ERA_HISTORY if present. "
        "Tell it: 'You are in Result Mode. Here is the scout data and user interview answers.' "
        "WAIT for narrator_agent to return its JSON array before proceeding.\n"
        "STEP 3 — Only after narrator_agent has returned its JSON: Call `compliance_agent`. "
        "Pass narrator_agent's JSON array output for final review.\n"
